...
case $1 in
   f|fullcone)
      ...
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.2 --jump SNAT --to-source 200.0.0.1
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.3 --jump SNAT --to-source 200.0.0.2

      iptables --table nat --append PREROUTING --in-interface eth1 --destination 200.0.0.1 --jump DNAT --to-destination 10.0.0.2
      iptables --table nat --append PREROUTING --in-interface eth1 --destination 200.0.0.2 --jump DNAT --to-destination 10.0.0.3
      ;;
      ...
      cat /var/log/ulog/syslogemu.log | awk -W interactive '
         {
            for (i=1; i<=NF; i++) {
               ...
               if (fields[1]=="SRC") {
                  src=fields[2]
                  if (src=="10.0.0.2") { ext_src="200.0.0.1"; }
                  if (src=="10.0.0.3") { ext_src="200.0.0.2"; }
               }
            }
   ...
   s|symmetric)
      # Appear to the outside with fixed addresses
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.2 --destination 200.0.0.40 --jump SNAT --to-source 200.0.0.1
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.2 --destination 200.0.0.60 --jump SNAT --to-source 200.0.0.2
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.3 --destination 200.0.0.40 --jump SNAT --to-source 200.0.0.3
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 10.0.0.3 --destination 200.0.0.60 --jump SNAT --to-source 200.0.0.4
      ;;
